...
- **Disk filling up from accumulated Docker images.** On any Docker host, the most common culprit behind a full disk is `/var/lib/docker`: every build or deploy stacks on a new image while the old ones are never cleaned up. The fix is a scheduled `docker image prune` with a time filter — and absolutely no casual `docker system prune` on machines with data volumes; one mistaken `--volumes` flag and real data is gone. A related trap: `rm`-ing a huge log file without `df` budging — the file isn't really dead while a process still holds its fd, and `lsof +L1` names the culprit.
