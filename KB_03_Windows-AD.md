...
> **Vì sao:** tách ring + chuyển ngữ cảnh có kiểm soát ngăn code user-mode tùy tiện đọc/ghi RAM kernel hay phần cứng — nền tảng của mọi cô lập bảo mật.
...
> **Lưu ý:** trường `InheritedFromUniqueProcessId` trong EPROCESS chính là PPID mà Sysmon ghi vào Event ID 1 — quan trọng để phát hiện parent-child bất thường (ví dụ `winword.exe` sinh `cmd.exe`).
...
> **Vì sao:** `svchost.exe -k netsvcs` gom nhiều service nhẹ vào một process để tiết kiệm tài nguyên. Tác dụng phụ bảo mật: nhiều service chạy chung token → bề mặt lớn, khó phân biệt service nào lỗi.

**Lưu ý:** Event ID **7045** (System log) = "A new service was installed" — tín hiệu mạnh cho lateral movement (PsExec tạo service `PSEXESVC`). **Unquoted service path** + ghi được vào thư mục cha = leo thang quyền cổ điển.
...
**Lưu ý:** Event ID **4698** (task created), **4702** (updated) trong Security log; **106/200/201** trong `Microsoft-Windows-TaskScheduler/Operational`. Chạy `SYSTEM` + trigger logon = persistence cần soi.
...
> **Lưu ý:** săn brute-force/password spraying. Nhiều 4625 `0xC000006A` từ một IP tới nhiều TargetUserName là spraying; tới một user nhiều lần là brute force.
...
> **Lưu ý:** 1102 (Security) và 104 (System, log khác bị clear) là chỉ báo anti-forensic. Forward log realtime (WEF/Sysmon→SIEM) trước khi attacker clear là biện pháp đối phó.
...
> **Lưu ý:** ProcessGuid bền vững (không bị tái dùng như PID) → nối toàn bộ chuỗi event của một process. Event 8/10 với GrantedAccess `0x1410`/`0x1010` tới `lsass.exe` là chỉ báo credential dumping mạnh.
...
| **Forest** | Ranh giới tin cậy cao nhất. Chia sẻ schema chung + Global Catalog + Enterprise Admins. **Forest mới là ranh giới tin cậy thật sự** |
...
Cây phân cấp luận lý của AD: **Forest** bao toàn bộ và là ranh giới tin cậy cao nhất; bên trong là một hoặc nhiều **Tree** (cùng namespace liên tục), mỗi Tree chứa các **Domain**; trong Domain phân nhánh thành **OU** để áp GPO và ủy quyền, chứa các object lá (user/computer/group).

```
FOREST  (ranh giới tin cậy cao nhất — schema + Global Catalog + Enterprise Admins)
└── TREE: example.com
...
Lưu ý ranh giới tin cậy: OU **không** phải ranh giới tin cậy (chỉ là đơn vị áp chính sách/ủy quyền); Domain là ranh giới quản trị nhưng **Forest mới là ranh giới tin cậy thật sự** — chiếm một domain trong forest có thể bắc cầu sang domain khác qua trust nếu không bật SID filtering.
...
> **Vì sao:** schema replicate toàn forest và khó sửa vì một thay đổi schema là vĩnh viễn (chỉ deactivate, không xóa hẳn) và ảnh hưởng mọi DC — nên forest là ranh giới schema.
...
> **Lưu ý:** SYSVOL đọc được bởi mọi Authenticated User. Lịch sử: GPP (`Groups.xml`) từng chứa mật khẩu mã hóa AES với **key công khai** (CVE-2014-1812) → ai cũng giải được. Săn file `cpassword` trong SYSVOL.
...
| Realm | Một/hai chiều | Không | Tới Kerberos realm phi-Windows |
...
| 0x0200 | NORMAL_ACCOUNT | Tài khoản user thường |
| 0x10000 | DONT_EXPIRE_PASSWORD | Mật khẩu không hết hạn |
...
> **Lưu ý:** LDAP 389 cleartext lộ bind credential (simple bind). Microsoft khuyến nghị **LDAP signing + channel binding** (LDAPS) để chống MITM/relay. Truy vấn LDAP bất thường (enum toàn bộ user/SPN) là dấu hiệu recon — phát hiện qua audit "Directory Service Access" (Event 4662) hoặc network monitoring.
...
Kerberos v5 (RFC 4120) là giao thức xác thực mặc định trong AD, dựa trên ticket và **mã hóa đối xứng**. Thành phần:
...
```cmd
:: Liệt kê ticket Kerberos đang cache trong phiên
klist

:: Xóa ticket (buộc xin lại) — hữu ích khi debug
klist purge
```
...
> **Lưu ý:** Kerberoasting để lại **Event 4769** với `TicketEncryptionType=0x17` (RC4) và `ServiceName` là tài khoản user-SPN. Nhiều 4769 RC4 trong thời gian ngắn từ một account = báo động.
...
> **Lưu ý:** server không bao giờ thấy mật khẩu, chỉ thấy NT hash gián tiếp qua response. Nhưng nếu attacker có **NT hash**, họ tính được response mà không cần mật khẩu → đây chính là cơ sở của **pass-the-hash**.

### 3.10.2. So sánh NTLM vs Kerberos

| Tiêu chí | NTLM | Kerberos |
|----------|------|----------|
| Mô hình | Challenge/response | Ticket |
...
**Dấu hiệu**: ticket Kerberos dùng từ máy/IP không khớp với 4768 ban đầu; thiếu 4768 tương ứng với hoạt động Kerberos.
...
| Tiêu chí | Golden Ticket | Silver Ticket |
|---|---------------|---------------|
...
> **Vì sao:** golden ticket toàn năng vì TGT được ký bằng krbtgt key. Nếu attacker có krbtgt hash, họ tự phát hành TGT với PAC chứa SID Domain Admins, hạn dùng 10 năm. Chỉ reset krbtgt (2 lần) mới vô hiệu.
...
> **Vì sao:** 4104 mạnh vì attacker hay dùng `powershell -enc <base64>` để giấu lệnh. Script Block Logging ghi nội dung **sau khi giải mã**, nên thấy lệnh thật. Event 4104 với `Level=Warning` được engine tự đánh dấu khi phát hiện pattern khả nghi (Invoke-Mimikatz, IEX, DownloadString...).
...
> **Lưu ý:** attacker downgrade về PowerShell v2 (`powershell -version 2`) để né script block logging (v2 không hỗ trợ). Phòng: gỡ .NET 2.0/PowerShell v2 engine. AMSI (Antimalware Scan Interface) bổ sung quét nội dung script lúc runtime; attacker dùng AMSI bypass → giám sát chuỗi bypass đặc trưng.
...
> **Lưu ý:** attribute `ms-Mcs-AdmPwd` được bảo vệ bằng ACL + thường đánh dấu "confidential". Phải kiểm tra **không vô tình cấp quyền đọc** cho nhóm rộng. Audit đọc LAPS (Event 4662 trên attribute đó).
...
> **Cảnh báo:** nếu Sysmon Event 10 vẫn thấy handle PROCESS_VM_READ tới lsass.exe khi RunAsPPL bật, nghĩa là attacker đã nạp driver để gỡ bảo vệ PPL (BYOVD) — báo động cao.
