...
Trên AWS EC2 (IMDSv1), endpoint metadata trả về credential tạm thời:
...
```
Payload phát hiện (Jinja2):
```
name={{7*7}}      → render ra "49"  ⇒ template engine đang đánh giá biểu thức
```
Payload leo thang tới RCE (Jinja2/Python):
```
...
Công cụ sinh payload — **ysoserial**:
```bash
