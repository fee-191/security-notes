...
**GitOps và Argo CD.** GitOps coi **Git là nguồn sự thật duy nhất (single source of truth)** cho trạng thái mong muốn của hệ thống. **Argo CD** là agent chạy *trong* cluster Kubernetes, liên tục so sánh trạng thái khai báo trong Git với trạng thái thực tế và reconcile (đồng bộ) cho khớp.
