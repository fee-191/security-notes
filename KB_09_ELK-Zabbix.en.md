...
- **Elasticsearch**: a Lucene-based **search engine** that uses an **inverted index** to perform full-text lookups across billions of records with millisecond latency — something a relational database cannot deliver.
...
This chapter dives deep into two tool families whose roles are frequently confused in operational environments: the **ELK/Elastic Stack** (a platform for collecting, storing, querying, and visualizing logs/events at large scale) and **Zabbix** (a platform for monitoring infrastructure and performance using a metric/threshold model). The goal of the chapter is not to "introduce products" but to describe them down to the level of **wire format, on-disk record structure, every configuration field, and every processing step** — enough for a Blue Team/AppSec/DevSecOps engineer to operate, debug, and assess risk for real.

There is one fundamental distinction that must be grasped from the very start:
...
Remember: Elasticsearch **is a search engine**, not a relational database; Zabbix **is a monitoring system**, not a log store. Every design decision below stems from these two essential natures.
...
Elasticsearch is built on the **Apache Lucene** library. Its central data structure is the **inverted index**. In a relational database, you go from row → column → value (forward). The inverted index reverses this: it goes from **term (word) → list of documents containing that term**. This is why full-text search across billions of documents still runs at millisecond latency: instead of a linear scan, you look up the term and then union/intersect the posting lists.
...
```
   ...
   │ (collects 1   │   │ (on the host) │    │  agentless     │
   │  zone)        │   │               │    │                │
   └───────▲───────┘   └───────────────┘    └────────────────┘
           │ TCP 10050
   ┌───────┴───────┐
