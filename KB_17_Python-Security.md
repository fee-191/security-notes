...
**Định nghĩa cốt lõi.** Tự động hóa bảo mật là quá trình chuyển các tác vụ thu thập, phân tích, ra quyết định và phản ứng sự cố thành mã chạy được, có thể lặp lại và kiểm chứng. Python đóng vai trò **ngôn ngữ keo (glue language)**: phần logic viết bằng Python, phần tính toán nặng giao cho thư viện C đã tối ưu và được audit.
...
Python được chọn làm "ngôn ngữ keo" (glue language) của bảo mật vì ba lý do kỹ thuật cụ thể, không phải vì "dễ học":
...
| `memoryview` | View không sao chép lên buffer | — | Xử lý buffer lớn không copy |
...
print(len(s), len(b))        # 5 5? KHÔNG: 5 4 -> thực ra 5 ký tự, 6 byte
...
**LƯU Ý BẢO MẬT:** Khi so sánh token/MAC, KHÔNG dùng `==` trên `str`/`bytes` vì nó short-circuit (dừng sớm khi gặp byte khác → timing leak). Dùng `hmac.compare_digest()` (xem 17.13).
...
**LƯU Ý BẢO MẬT (supply chain):**
- Luôn **pin version** (`==`) trong môi trường production; range mở (`>=`) cho phép kẻ tấn công đẩy bản độc qua dependency confusion.
...
| Data Offset (4 bit) + Reserved + Flags | 2 byte | 12 | Độ dài header + 9 cờ | SYN=0x002 |
| Window | 2 byte | 14 | Kích thước cửa sổ nhận | 65535 |
| Checksum | 2 byte | 16 | Tổng kiểm tra | |
...
def create_ticket(sess, project="SEC", summary="Suspicious login", desc=""):
    ...
    r = sess.post(
        ...
        timeout=(3.05, 10),            # (connect timeout, read timeout) - LUÔN đặt
        verify=True,                   # KHÔNG bao giờ verify=False trong production
    )
...
Output mẫu khi alert "brute-force phát hiện" được push: Slack hiển thị attachment màu đỏ với text mô tả IP và số lần thử.
...
**Phân tích pattern byte-level:** `\d{1,3}(?:\.\d{1,3}){3}` khớp octet 1–3 chữ số, lặp 3 lần với dấu chấm. Lưu ý nó KHÔNG kiểm tra octet ≤ 255 — chấp nhận `999.999.999.999`. Để validate đúng nên dùng `ipaddress.ip_address()` sau khi trích.

**LƯU Ý BẢO MẬT — ReDoS (Regex Denial of Service):**
...
### VÍ DỤ THỰC TẾ — cách ĐÚNG và SAI
...
# subprocess.run(f"ping -c1 {host}", shell=True)   # ĐỪNG LÀM
...
**LƯU Ý BẢO MẬT:** scapy cần `CAP_NET_RAW` (root). Trong Docker phải chạy `--cap-add=NET_RAW` hoặc `--privileged`. Gửi packet thô tới mạng không được phép là tấn công.
...
**Cơ chế:** placeholder (`?` cho sqlite3, `%s` cho psycopg2) khiến driver gửi câu lệnh và dữ liệu riêng biệt (prepared statement). DB phân tích cú pháp câu lệnh trước, dữ liệu được treat thuần như giá trị → không thể đổi cấu trúc query. **KHÔNG dùng f-string/`%`/`.format()` để chèn dữ liệu vào SQL.**
...
4. Crypto: `secrets` cho token, `hmac.compare_digest` để so sánh, Argon2/bcrypt cho mật khẩu, KHÔNG MD5/SHA-1 cho integrity.
